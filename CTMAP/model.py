            ...
            for minibatch_id, ((rna_data, rna_label, id_rna), (st_data, st_label, id_st)) in enumerate(iters):
                ...
                if iteration == maxiter:
                    with torch.no_grad():
                        ...
                        # 构建局部整数 → 全局整数的映射（关键！）
                        local_to_global = dict(zip(unique_cell_types, unique_cell_types))  # 局部整数就是全局整数（因为 truth_rna 来自全局编码）
                        ...
                        for _, ((st_data, st_label, id_st)) in enumerate(st_test_loader):
                            ...
                            batch_indices.append(id_st.cpu().numpy())

                            distances = cdist(z_st.cpu().numpy(), cell_type_means, metric='euclidean')
                            closest_cell_type_indices = np.argmin(distances, axis=1)
                            pred_local = unique_cell_types[closest_cell_type_indices]
                            # 转成全局整数
                            pred_global = np.array([local_to_global[l] for l in pred_local])
                            pred_label.append(pred_global)
                        ...
                        truth_label = truth_label[sorted_order]
                        pred_label = pred_label[sorted_order]
                        st_embeddings = st_embeddings[sorted_order]
