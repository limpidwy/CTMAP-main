...
def cell_type_encoder(adata_rna, adata_spa):
    # 提取细胞类型
    rna_celltype = adata_rna.obs['cell_type'].values.astype(str)  # 确保字符串
    st_celltype = adata_spa.obs['cell_type'].values.astype(str)
    ...
    # ===== 新增部分：返回完整的整数 → 原始字符串名称 映射字典 =====
    # full_label_to_name: {0: 'Astrocyte', 1: 'Endothelial', ...}
    full_label_to_name = inverse_full_label_mapping

    # 原有返回不变，只在最后额外返回映射字典
    return rna_labels, seqfish_labels, cell_types, full_label_to_name
